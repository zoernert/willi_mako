...
export const ChatSelectorDialog: React.FC<ChatSelectorDialogProps> = ({
  ...
  const loadChats = async () => {
    ...
    try {
      ...
      console.log('Versuche Chats zu laden...');
      ...
      if (!response.ok) {
        ...
        throw new Error(`Fehler beim Laden der Chats: ${response.status} ${response.statusText}`);
      }
      
      const data = await response.json();
      console.log('Geladene Chats (Datentyp):', typeof data, Array.isArray(data));
      console.log('Geladene Chats (Struktur):', Object.keys(data));
      
      // Prüfen, ob die Datenstruktur wie erwartet ist
      if (data && Array.isArray(data.chats)) {
        console.log(`${data.chats.length} Chats geladen`);
        setChats(data.chats);
      } else if (data && Array.isArray(data)) {
        console.log(`${data.length} Chats geladen (direktes Array)`);
        setChats(data);
      } else {
