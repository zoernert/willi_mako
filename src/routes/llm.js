...
function generateEmailTemplate(clarification, requestType, language) {
  ...
  const companyName = clarification.marketPartner?.companyName || 'Marktpartner';
  const dar = clarification.dataExchangeReference?.dar || 'N/A';
  ...
  if (isDataIssue) problemCategory = 'Datenabweichung';
  else if (isProcessIssue) problemCategory = 'Prozessklärung';
  else if (isBillingIssue) problemCategory = 'Abrechnungsklärung';
