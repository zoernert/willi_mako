...
function baseEmailLayout(title: string, contentHtml: string): string {
...
function makeStarLinks(baseUrl: string, params: Record<string,string|number>): string {
  const rows: string[] = [];
  for (let i=1;i<=5;i++) {
...
router.get('/public/feedback', asyncHandler(async (req: Request, res: Response) => {
  const { userId, type, chatId, stars, useful } = req.query as any;
  ...
  const inserted = await DatabaseHelper.executeQuerySingle<{ id: string }>(
    `INSERT INTO engagement_feedback (user_id, type, chat_id, stars, useful, user_agent, ip_address)
     VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`,
    [userId, type, chatId || null, starsNum, usefulVal, req.headers['user-agent'] || null, req.ip]
  );

  // Optional: update status of engagement email when feedback given
  await DatabaseHelper.executeQuery(
    `UPDATE engagement_emails SET status = 'responded' WHERE user_id = $1 AND type = $2 AND (chat_id IS NOT DISTINCT FROM $3)`,
    [userId, type, chatId || null]
  );

  // Branded thank you page with comment form
  const starVisual = '★'.repeat(starsNum) + '☆'.repeat(5 - starsNum);
  const frontendUrl = await SystemSettingsService.getSetting('system.frontend_url', 'https://stromhaltig.de');
  const content = `
    ...
    <form action="/api/engagement/public/feedback" method="POST" style="margin-top:16px">
      <input type="hidden" name="feedbackId" value="${inserted?.id || ''}" />
...
router.post('/public/feedback', asyncHandler(async (req: Request, res: Response) => {
  const { feedbackId, userId, type, chatId, stars, useful, comment } = req.body as any;
  ...
  if (sanitizedComment) {
    if (feedbackId) {
      await DatabaseHelper.executeQuery(
        `UPDATE engagement_feedback SET comment = $1 WHERE id = $2`,
        [sanitizedComment, feedbackId]
      );
    } else {
      await DatabaseHelper.executeQuery(
        `UPDATE engagement_feedback SET comment = $1 
         WHERE id = (
...
router.post('/admin/engagement/send', authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  if (req.user?.role !== 'admin') return res.status(403).json({ success: false, message: 'Forbidden' });

  // Feature flag check
  const enabled = await SystemSettingsService.getSetting('engagement.enabled', true);
  if (!enabled) {
    return res.status(503).json({ success: false, message: 'Engagement emails disabled' });
  }

  const { userId, type, chatId } = req.body as { userId: string; type: 'layout_feedback'|'chat_feedback'; chatId?: string };

  if (!userId || !type) return res.status(400).json({ success: false, message: 'Missing userId or type' });
  if (type === 'chat_feedback' && !chatId) return res.status(400).json({ success: false, message: 'chatId required for chat_feedback' });

  // Resolve user info
  const user = await DatabaseHelper.executeQuerySingle<{ id: string; email: string; full_name: string | null }>(
  ...
  if (type === 'chat_feedback' && chatId) {
    const chat = await DatabaseHelper.executeQuerySingle<{ id: string }>(
      'SELECT id FROM chats WHERE id = $1 AND user_id = $2', [chatId, user.id]
    );
    if (!chat) return res.status(400).json({ success: false, message: 'Chat not found for user' });
  }

  // Check if already sent (unique constraint also protects)
  const already = await DatabaseHelper.executeQuerySingle<{ id: string }>(
    'SELECT id FROM engagement_emails WHERE user_id=$1 AND type=$2 AND (chat_id IS NOT DISTINCT FROM $3)',
    [user.id, type, chatId || null]
  );
  if (already) {
  ...
  const frontendUrl = await SystemSettingsService.getSetting('system.frontend_url', process.env.FRONTEND_URL || 'https://stromhaltig.de');
  ...
  const sendInfo = await emailService.sendEmailWithInfo({ to: user.email, subject, html });

  const rec = await DatabaseHelper.executeQuerySingle<{ id: string }>(
    'INSERT INTO engagement_emails (user_id, type, chat_id, subject, message_id, status) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id',
    [user.id, type, chatId || null, subject, sendInfo?.messageId || null, 'sent']
  );

  res.json({ success: true, id: rec?.id, messageId: sendInfo?.messageId });
}));

// Admin: list engagement status for a user (include latest feedback)
router.get('/admin/engagement/:userId', authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  if (req.user?.role !== 'admin') return res.status(403).json({ success: false, message: 'Forbidden' });
  const { userId } = req.params;
  const rows = await DatabaseHelper.executeQuery<any>(
    `SELECT e.id, e.type, e.chat_id, e.status, e.sent_at,
            (
