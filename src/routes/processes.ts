...
router.post('/search', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const result = await ProcessService.searchProcesses(query, conversationHistory);
...
router.post('/optimize', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    console.log(`[ProcessOptimize] User: ${req.user?.id}, Optimization: "${optimizationRequest}"`);

    const result = await ProcessService.optimizeProcess(
      originalQuery, 
      optimizationRequest, 
      currentDiagrams
    );
...
router.get('/health', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const testResult = await ProcessService.searchProcesses('test', []);
