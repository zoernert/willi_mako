...
export default function createQuizRoutes(db: Pool) {
  ...
  // Get quiz results
  router.get('/attempts/:attemptId/results', authenticateToken, async (req: AuthenticatedRequest, res) => {
    try {
  ...
  router.post('/leaderboard-settings', authenticateToken, async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'User not authenticated' });
