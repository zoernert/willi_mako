 ...
 */
router.put('/users/:userId/cs30-access', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { userId } = req.params;
  const { canAccess } = req.body;

  if (typeof canAccess !== 'boolean') {
    throw new AppError('canAccess must be a boolean value', 400);
  }

  try {
    // Check if user exists
    const userExists = await DatabaseHelper.executeQuerySingle<{count: number}>('SELECT COUNT(*) as count FROM users WHERE id = $1', [userId]);
    if (!userExists || userExists.count === 0) {
      throw new AppError('User not found', 404);
    }

    await DatabaseHelper.executeQuery(
      'UPDATE users SET can_access_cs30 = $1, updated_at = NOW() WHERE id = $2', 
      [canAccess, userId]
    );
    ...
    throw new AppError('Failed to update CS30 access', 500);
  }
}));
 ...
 */
router.get('/users/:userId/details', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { userId } = req.params;
  ...
  try {
    ...
    const m2cRoles = await DatabaseHelper.executeQuery<{ role_name: string }>(`
      SELECT role_name
      FROM m2c_roles
      WHERE id = ANY($1::uuid[])
    ...
    // Get chat statistics
    const chatStats = await DatabaseHelper.executeQuerySingle<{
      chat_count: string;
    ...
    // Get recent chats
    const recentChats = await DatabaseHelper.executeQuery<{
      id: string;
      title: string;
      created_at: Date;
      updated_at: Date;
    ...
    // Get quiz statistics
    const quizStats = await DatabaseHelper.executeQuerySingle<{
    ...
    const userDetails = {
      ...
      chat_count: parseInt(chatStats?.chat_count || '0'),
      message_count: parseInt(chatStats?.message_count || '0'),
      recent_chats: recentChats.map(chat => ({
        ...chat,
        message_count: parseInt(chat.message_count)
      })),
      completed_quizzes: parseInt(quizStats?.completed_quizzes || '0'),
      avg_quiz_score: quizStats?.avg_quiz_score ? Math.round(quizStats.avg_quiz_score) : null
    };

    ResponseUtils.success(res, userDetails, 'User details retrieved successfully');
  } catch (error) {
