...
router.get('/:teamId', async (req, res) => {
    try {
        ...
        // Berechtigung prüfen
        if (!req.user.isAdmin && !await hasTeamAccess(req.user.id, teamId)) {
...
router.put('/:teamId', async (req, res) => {
    try {
        ...
        // Berechtigung prüfen
        if (!req.user.isAdmin && !await hasTeamAdminAccess(req.user.id, teamId)) {
        ...
        try {
            ...
            if (imapPassword) {
                encryptedPassword = ImapEmailService.encryptPassword(imapPassword);
            }
...
async function hasTeamAdminAccess(userId, teamId) {
    try {
        const result = await pool.query(`
            SELECT 1 FROM team_members 
            WHERE user_id = $1 AND team_id = $2 AND role IN ('admin', 'leader')
        `, [userId, teamId]);
