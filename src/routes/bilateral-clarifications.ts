...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    const queryParams: any[] = [req.user?.id, req.user?.teamId];
    let paramCounter = 3;
    ...
    if (caseType && Array.isArray(caseType)) {
      ...
      queryParams.push(caseType as string[]);
      paramCounter++;
    }
    ...
    if (createdBy) {
      ...
      queryParams.push(createdBy as string);
      paramCounter++;
    }
    ...
    if (tags && Array.isArray(tags)) {
      ...
      queryParams.push(tags as string[]);
      paramCounter++;
    }
    ...
    if (dateRangeStart) {
      ...
      queryParams.push(dateRangeStart as string);
      paramCounter++;
    }

    if (dateRangeEnd) {
      query += ` AND bc.created_at <= $${paramCounter}`;
      queryParams.push(dateRangeEnd as string);
      paramCounter++;
    }
    ...
    const sortColumn = allowedSortFields.includes(sortField as string) ? 
