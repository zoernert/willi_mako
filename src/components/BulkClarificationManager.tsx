...
const BulkClarificationManager: React.FC<BulkClarificationManagerProps> = ({
  ...
  const handleUpdateItem = async (itemId: string, updates: Partial<BulkClarificationItem>) => {
    if (!clarificationId) return;

    try {
      const response = await fetch(`/api/clarifications/${clarificationId}/bulk-items/${itemId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleBatchStatusUpdate = async (newStatus: string) => {
    if (!clarificationId || selectedItems.size === 0) return;
    ...
    try {
      ...
      const response = await fetch(`/api/clarifications/${clarificationId}/bulk-items/batch-update`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      setError('Fehler beim Batch-Update');
    }
  };
