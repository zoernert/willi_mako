...
class ImapEmailService {
  ...
  async processQueuedEmail(queueId) {
    ...
    try {
      ...
      await client.query(
        'UPDATE email_processing_queue SET extracted_references = $1 WHERE id = $2',
        [JSON.stringify(extractedData), queueId]
      );
      ...
      if (this.shouldCreateClarification(emailData, extractedData)) {
        ...
        if (clarification) {
          await client.query(
            'UPDATE email_processing_queue SET created_clarification_id = $1, processing_status = $2, processed_at = NOW() WHERE id = $3',
            [clarification.id, 'COMPLETED', queueId]
          );
  ...
  shouldCreateClarification(emailData, extractedData) {
    ...
    const hasReferences = extractedData.referenzen && (
      extractedData.referenzen.vorgangsnummern?.length > 0 ||
      extractedData.referenzen.zählpunkte?.length > 0 ||
      extractedData.referenzen.lieferstellen?.length > 0
    );
    
    const hasCategory = extractedData.klassifikation?.kategorie && 
  ...
  async updateLastProcessedUid(teamId, uid) {
    try {
      await this.pool.query(
        'UPDATE team_email_configs SET last_processed_uid = $1 WHERE team_id = $2',
        [uid.toString(), teamId]
      );
    } catch (error) {
