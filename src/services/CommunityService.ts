...
export class CommunityService {
   ...
   */
  async deleteThread(threadId: string, adminUserId: string): Promise<void> {
    ...
    try {
      ...
      await client.query(
        'DELETE FROM community_initiatives WHERE thread_id = $1',
        [threadId]
      );
      
      // Delete comments (will be cascade deleted, but explicit for clarity)
      await client.query(
        'DELETE FROM document_comments WHERE thread_id = $1',
        [threadId]
      );
      
      // Delete the thread itself
      await client.query(
        'DELETE FROM community_threads WHERE id = $1',
        [threadId]
      );
