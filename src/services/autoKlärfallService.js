...
class AutoKlärfallService {
    ...
    async createClarificationFromEmail(email, extractedData, teamId) {
        ...
        try {
            ...
            const partnerId = await this.findOrCreateMarketPartner(
                extractedData.marktpartner, 
                client
            );

            // 2. Basis-Klärfall erstellen
            const clarification = await this.createBaseClarification(
                email, 
                extractedData, 
                teamId, 
                partnerId, 
    ...
    async createBaseClarification(email, extractedData, teamId, partnerId, client) {
        const clarificationData = {
            ...
            category: this.mapCategory(extractedData.klassifikation?.kategorie),
            priority: this.mapPriority(extractedData.klassifikation?.priorität),
        ...
        `, [
            ...
            this.mapEffort(extractedData.klassifikation?.arbeitsaufwand)
        ]);
    ...
    async executeAutomaticActions(clarification, extractedData, client) {
        try {
            ...
            if (extractedData.automatisierung?.autoBearbeitung) {
            ...
            if (extractedData.automatisierung?.standardAntwort) {
                const standardResponse = await this.llmService.suggestStandardResponse(
                    clarification.original_email, 
                    extractedData
                );
            ...
            if (extractedData.automatisierung?.weiterleitung?.erforderlich) {
    ...
    async assignToSpecialist(clarification, extractedData, client) {
        try {
            ...
            `, [clarification.team_id, extractedData.klassifikation?.kategorie]);
            ...
            if (specialist.rows.length > 0) {
                ...
                await this.logActivity(
                    clarification.id, 
                    'auto_assigned', 
                    `Automatisch zugewiesen an ${specialist.rows[0].name}`,
                    client
                );
            }
    ...
    async createBulkClarification(email, listItems, extractedData, teamId) {
        ...
        try {
            ...
            const mainClarification = await this.createBaseClarification(
                email, 
                { ...extractedData, type: 'sammelklärung' }, 
                teamId, 
                null, 
                client
            );
    ...
    generateTitle(subject, extractedData) {
        const category = extractedData.klassifikation?.kategorie || 'Allgemein';
        const partner = extractedData.marktpartner?.name || 'Unbekannt';
    ...
    generateDescription(email, extractedData) {
        ...
        if (extractedData.inhalt?.zusammenfassung) {
            description += `**Zusammenfassung:** ${extractedData.inhalt.zusammenfassung}\n\n`;
        }
        
        if (extractedData.inhalt?.klärungspunkte?.length > 0) {
        ...
        if (extractedData.inhalt?.nächsteSchritte?.length > 0) {
