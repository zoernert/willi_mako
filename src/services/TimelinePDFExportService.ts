...
export class TimelinePDFExportService {
  private generateHTML(data: TimelineExportData): string {
...
<body>
  ...
  async generatePDF(data: TimelineExportData): Promise<Buffer> {
    let browser;
  ...
  async exportTimelineToPDF(data: TimelineExportData): Promise<Buffer> {
