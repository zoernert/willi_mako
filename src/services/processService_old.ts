    ...
    try {
      ...
      const diagrams: MermaidDiagram[] = await Promise.all(
        mermaidResults.map(async (result: any, index: number) => {
          const originalCode = result.payload?.mermaid_code || '';
          const title = result.payload?.context_text || `Prozessdiagramm ${index + 1}`;
...
Format: Einfache Liste ohne Nummerierung.`;
   ...
   * Improve Mermaid code using LLM
   */
  static async improveMermaidCode(
    originalCode: string, 
    title?: string, 
    context?: string
  ): Promise<MermaidCodeImproveResult> {
    try {
...
`;
      ...
      const improvedResponse = await geminiService.generateResponse(improvementPrompt);
