...
class GoogleAIKeyManager {
   ...
   */
  updateRateLimits() {
  if (!this.freeEnabled) return; // Skip tracking when free is disabled
  ...
  if (!this.freeEnabled) return false;
      ...
      try {
        ...
        if (savedMetrics) {
          ...
          if (!this.usageMetrics.providers) {
            ...
            // Ensure keys present
            if (!this.usageMetrics.providers.gemini) this.usageMetrics.providers.gemini = { dailyUsage: {}, totalUsage: 0, currentDayUsage: 0 };
            if (!this.usageMetrics.providers.mistral) this.usageMetrics.providers.mistral = { dailyUsage: {}, totalUsage: 0, currentDayUsage: 0 };
          }
  ...
  getUsageMetrics() {
    ...
    if (!this.usageMetrics.providers) this.usageMetrics.providers = { gemini: { dailyUsage: {}, totalUsage: 0, currentDayUsage: 0 }, mistral: { dailyUsage: {}, totalUsage: 0, currentDayUsage: 0 } };
    ...
    if (!this.usageMetrics.providers.gemini.dailyUsage) this.usageMetrics.providers.gemini.dailyUsage = {};
    if (!this.usageMetrics.providers.mistral.dailyUsage) this.usageMetrics.providers.mistral.dailyUsage = {};
