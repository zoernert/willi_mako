...
async function getAllPublicFAQs() {
    ...
    `);
        const faqs = await Promise.all(result.rows.map(async (faq) => {
            ...
            return Object.assign(Object.assign({}, faq), { tags: parsedTags, slug, created_at: faq.created_at ? faq.created_at.toISOString() : new Date().toISOString(), updated_at: faq.updated_at ? faq.updated_at.toISOString() : new Date().toISOString(), related_faqs: relatedFAQs });
        }));
        return faqs;
    }
    catch (error) {
...
async function getFAQsByTag(tag) {
    ...
    `, [tag]);
        return Promise.all(result.rows.map(async (faq) => {
            const slug = generateFAQSlug(faq.title);
            const relatedFAQs = await getRelatedFAQs(faq.id, faq.content + ' ' + faq.answer);
            return Object.assign(Object.assign({}, faq), { slug, created_at: faq.created_at ? faq.created_at.toISOString() : new Date().toISOString(), updated_at: faq.updated_at ? faq.updated_at.toISOString() : new Date().toISOString(), related_faqs: relatedFAQs });
        }));
    }
    catch (error) {
...
async function getLatestFAQs(limit = 20) {
    ...
    `, [limit]);
        return Promise.all(result.rows.map(async (faq) => {
            ...
            return Object.assign(Object.assign({}, faq), { slug, related_faqs: relatedFAQs });
        }));
    }
    catch (error) {
