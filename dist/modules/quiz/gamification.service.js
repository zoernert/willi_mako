...
class GamificationService {
    async awardBadges(userId, context) {
    ...
    async checkIfUserHasBadge(userId, badgeName) {
        const query = `
            SELECT 1 FROM user_badges ub
            JOIN badges b ON ub.badge_id = b.id
            WHERE ub.user_id = $1 AND b.name = $2
        `;
        const result = await database_1.DatabaseHelper.executeQuerySingle(query, [userId, badgeName]);
        return !!result;
    }
    async grantBadge(userId, badgeName, description) {
        // First, find or create the badge
        let badge = await database_1.DatabaseHelper.executeQuerySingle('SELECT * FROM badges WHERE name = $1', [badgeName]);
        if (!badge) {
            badge = await database_1.DatabaseHelper.executeQuerySingle('INSERT INTO badges (name, description, icon_url) VALUES ($1, $2, $3) RETURNING *', [badgeName, description, 'default_icon.png']);
        }
        if (badge) {
            // Grant the badge to the user
            await database_1.DatabaseHelper.executeQuery('INSERT INTO user_badges (user_id, badge_id) VALUES ($1, $2)', [userId, badge.id]);
            return badge;
        }
        return null;
    }
    async getLeaderboard(topic, limit = 10) {
        let query = `
     ...
     */
    async awardDocumentUsagePoints(documentId, chatId) {
        ...
        try {
            ...
            const documentResult = await client.query('SELECT COALESCE(uploaded_by_user_id, user_id) as uploader_id, is_processed FROM user_documents WHERE id = $1', [documentId]);
            if (documentResult.rows.length === 0) {
                console.warn(`Document with ID ${documentId} not found when awarding usage points`);
                throw new errors_1.AppError(`Document not found: ${documentId}`, 404);
            }
            ...
            const existingUsage = await client.query('SELECT 1 FROM document_usage_points WHERE document_id = $1 AND used_in_chat_id = $2', [documentId, chatId]);
            ...
            await client.query(`INSERT INTO document_usage_points (document_id, uploader_user_id, used_in_chat_id, points_awarded)
                 VALUES ($1, $2, $3, 1)`, [documentId, uploaderUserId, chatId]);
            // Award 1 point to uploader with 30-day expiry
            await client.query(`INSERT INTO user_points (user_id, points, source_type, description, expires_at)
                 VALUES ($1, 1, 'document_usage', 'Document used in AI response', CURRENT_TIMESTAMP + INTERVAL '30 days')`, [uploaderUserId]);
    ...
    async getTeamLeaderboard(teamId, limit = 10) {
        const client = await database_2.default.connect();
        try {
            const result = await client.query(`SELECT u.id as user_id, u.full_name as user_name, 
    ...
    async cleanupExpiredPoints() {
        const client = await database_2.default.connect();
        try {
            const result = await client.query('DELETE FROM user_points WHERE expires_at < CURRENT_TIMESTAMP');
     ...
     */
    async getUserValidPoints(userId) {
        const client = await database_2.default.connect();
        try {
            const result = await client.query('SELECT COALESCE(SUM(points), 0) as total_points FROM user_points WHERE user_id = $1 AND expires_at > CURRENT_TIMESTAMP', [userId]);
    ...
    async getUserPointsHistory(userId, limit = 50) {
        const client = await database_2.default.connect();
        try {
            const result = await client.query(`SELECT points, source_type, description, earned_at, expires_at,
