...
class UserPreferencesRepository {
    async getFlipModePreferences(userId) {
        ...
        return database_1.DatabaseHelper.executeQuerySingle(sql, [userId]);
    }
    async upsertFlipModePreferences(preferences) {
        const { user_id, energy_type, stakeholder_perspective, context_specificity, detail_level, topic_focus, } = preferences;
        const sql = `
            ...
            RETURNING *;
        `;
        const result = await database_1.DatabaseHelper.executeQuerySingle(sql, [
            user_id,
            energy_type,
    ...
    async getUserPreferences(userId) {
        ...
        const result = await database_1.DatabaseHelper.executeQuerySingle(sql, [userId]);
        if (!result) {
            return null;
        }
        return {
    ...
    async upsertUserPreferences(preferences) {
        const { user_id, companies_of_interest, preferred_topics, notification_settings, } = preferences;
        const sql = `
...
exports.default = new UserPreferencesRepository();
