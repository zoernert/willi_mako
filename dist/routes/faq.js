...
`;
  ...
  if (search) {
    query += `;
    AND(title, ILIKE, $$, { queryParams, : .length + 1 }, OR, description, ILIKE, $$, { queryParams, : .length + 1 }, OR, answer, ILIKE, $$, { queryParams, : .length + 1 }, OR, context, ILIKE, $$, { queryParams, : .length + 1 }) `;
    queryParams.push(` % $;
    {
        search;
    }
  ...
  if (search) {
    countQuery += `;
    AND(title, ILIKE, $$, { countParams, : .length + 1 }, OR, description, ILIKE, $$, { countParams, : .length + 1 }, OR, answer, ILIKE, $$, { countParams, : .length + 1 }, OR, context, ILIKE, $$, { countParams, : .length + 1 }) `;
    countParams.push(` % $;
    {
        search;
    }
...
router.get('/faqs/:id', asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await pool.query(`;
    SELECT;
    id, title, description, context, answer, additional_info, tags,
        view_count, is_public, created_at, updated_at;
    FROM;
    faqs;
    WHERE;
...
router.post('/faqs/:id/start-chat', authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const faqResult = await pool.query(`;
    SELECT;
    title, description, context, answer;
    FROM;
    faqs;
    WHERE;
    id = $1;
...
router.post('/faqs', authenticateBearerToken('str0mda0'), asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await pool.query(`
    ...
    INSERT;
    INTO;
    faqs(title, description, context, answer, additional_info, tags, is_active, is_public, created_at, updated_at);
...
router.get('/faq-tags', asyncHandler(async (req: Request, res: Response) => {
  const result = await pool.query(`;
    ...
    DISTINCT;
    tag;
    FROM(SELECT, jsonb_array_elements_text(tags), FROM, faqs, WHERE, is_active = true);
    ORDER;
    BY;
    tag `);
...
router.get('/admin/chats', authenticateToken, requireAdminForFaq, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const result = await pool.query(`;
    ...
    AND;
    role = 'assistant';
}), FROM, chats, c, JOIN, users, u, ON, c.user_id = u.id, LEFT, JOIN, messages, m, ON, c.id = m.chat_id, GROUP, BY, c.id, c.title, c.created_at, c.updated_at, u.full_name, u.email, HAVING, COUNT(m.id) > 0, ORDER, BY, c.updated_at, DESC, LIMIT, $1, OFFSET, $2 `, [limit, offset]);
...
router.get('/admin/chats/:chatId', authenticateToken, requireAdminForFaq, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const chatResult = await pool.query(`, SELECT, c.id, c.title, c.created_at, c.updated_at, u.full_name, u.email, FROM, chats, c, JOIN, users, u, ON, c.user_id = u.id, WHERE, c.id = $1 `, [chatId]);
  
  if (chatResult.rows.length === 0) {
    throw new AppError('Chat not found', 404);
  }
  
  const messagesResult = await pool.query(`, SELECT, id, role, content, created_at, FROM, messages, WHERE, chat_id = $1, ORDER, BY, created_at, ASC `, [chatId]);
...
router.post('/admin/chats/:chatId/create-faq', authenticateToken, requireAdminForFaq, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  // Get chat messages
  const messagesResult = await pool.query(`, SELECT, role, content, FROM, messages, WHERE, chat_id = $1, ORDER, BY, created_at, ASC `, [chatId]);
  ...
  const faqResult = await pool.query(`, INSERT, INTO, faqs(title, description, context, answer, additional_info, tags, source_chat_id, created_by, is_public), VALUES($1, $2, $3, $4, $5, $6, $7, $8, $9), RETURNING, id, title, description, context, answer, additional_info, tags, is_active, is_public, created_at `, [
    faqContent.title,
...
router.get('/admin/faqs', authenticateToken, requireAdminForFaq, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const result = await pool.query(`, SELECT, f.id, f.title, f.description, f.context, f.answer, f.additional_info, f.tags, f.is_active, f.is_public, f.view_count, f.created_at, f.updated_at, u.full_name, FROM, faqs, f, LEFT, JOIN, users, u, ON, f.created_by = u.id, ORDER, BY, f.created_at, DESC, LIMIT, $1, OFFSET, $2 `, [limit, offset]);
...
router.put('/admin/faqs/:id', authenticateToken, requireAdminForFaq, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const result = await pool.query(`, UPDATE, faqs, SET, title = $1, description = $2, context = $3, answer = $4, additional_info = $5, tags = $6, is_active = $7, updated_at = CURRENT_TIMESTAMP, WHERE, id = $8, RETURNING *
    `, [
    finalFAQData.title, 
...
router.get('/public/faqs', asyncHandler(async (req: Request, res: Response) => {
  ...
  let query = `, SELECT, id, title, description, context, answer, additional_info, tags, view_count, created_at, updated_at, FROM, faqs, WHERE, is_active = true, AND, is_public = true `;
  
  const queryParams: any[] = [];
  
  // Search functionality
  if (search) {
    query += `, AND(title, ILIKE, $$, { queryParams, : .length + 1 }, OR, description, ILIKE, $$, { queryParams, : .length + 1 }, OR, answer, ILIKE, $$, { queryParams, : .length + 1 }, OR, context, ILIKE, $$, { queryParams, : .length + 1 }) `;
  ...
  if (search) {
    countQuery += `, AND(title, ILIKE, $$, { countParams, : .length + 1 }, OR, description, ILIKE, $$, { countParams, : .length + 1 }, OR, answer, ILIKE, $$, { countParams, : .length + 1 }, OR, context, ILIKE, $$, { countParams, : .length + 1 }) `;
...
router.post('/admin/faqs/:id/artifacts/outline', authenticateToken, requireAdminForFaq, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const upsert = await pool.query(
    `, INSERT, INTO, faq_artifacts(faq_id, artifacts, meta), VALUES($1, $2, $3), ON, CONFLICT(faq_id), DO, UPDATE, SET, artifacts = $2, meta = $3, updated_at = CURRENT_TIMESTAMP, RETURNING, id, faq_id, artifacts, meta, created_at, updated_at `,
    [id, artifacts, meta]
  );
...
UPDATE;
SET;
artifacts = $2, updated_at = CURRENT_TIMESTAMP;
RETURNING;
id, faq_id, artifacts, meta, created_at, updated_at `,
    [id, artifacts]
  );
...
faq_artifacts;
SET;
artifacts = $1, updated_at = CURRENT_TIMESTAMP;
WHERE;
faq_id = $2;
RETURNING;
id, faq_id, artifacts, meta, created_at, updated_at `,
...
$;
{
    (Array.isArray(faq.tags) ? faq.tags : JSON.parse(faq.tags || '[]')).join(' ');
}
$;
{
    current.substring(0, 400);
}
`.trim();
