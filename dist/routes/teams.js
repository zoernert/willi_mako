...
router.get('/:teamId/email-config', auth_1.authenticateToken, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { teamId } = req.params;
    // Check team access
    const hasAccess = await teamService.hasTeamAccess(req.user.id, teamId);
    if (!hasAccess) {
        throw new errorHandler_1.AppError('Keine Berechtigung für dieses Team', 403);
    }
...
router.put('/:teamId/email-config', auth_1.authenticateToken, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { teamId } = req.params;
    ...
    if (!hasAdminAccess) {
        throw new errorHandler_1.AppError('Keine Admin-Berechtigung für dieses Team', 403);
    }
    // Validation
    if (autoProcessingEnabled && (!imapHost || !imapUsername)) {
        throw new errorHandler_1.AppError('IMAP-Host und Benutzername sind erforderlich für automatische Verarbeitung', 400);
    }
    const client = await database_1.default.connect();
    try {
...
router.get('/:teamId/email-processing/status', auth_1.authenticateToken, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { teamId } = req.params;
    // Check team access
    const hasAccess = await teamService.hasTeamAccess(req.user.id, teamId);
    if (!hasAccess) {
    ...
    // Get queue stats
    const queueResult = await database_1.default.query(`
    SELECT 
      processing_status,
      COUNT(*) as count
  ...
  `, [teamId]);
    ...
    LIMIT 10
  `, [teamId]);
    const queueStats = queueResult.rows.reduce((acc, row) => {
...
router.get('/cr-wmako-001/test', auth_1.authenticateToken, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    var _a;
    res.json({
        success: true,
...
router.post('/:teamId/bulk-clarifications', auth_1.authenticateToken, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { teamId } = req.params;
    const { title, description, category, priority, items } = req.body;
    const userId = req.user.id;
    try {
        // Validierung
...
router.get('/:teamId/bulk-clarifications', auth_1.authenticateToken, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { teamId } = req.params;
    const userId = req.user.id;
    try {
        // Berechtigung prüfen
